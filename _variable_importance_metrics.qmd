...
Variable importance is a way to measure how useful a variable is in predicting the target. This can be useful for both creating and analyzing models. Real world data often contains many potential predictors, and it can be difficult to know which variables should be considered important; variable importance metrics can help solve this problem.
...
3) How can this help us create the most accurate model possible?

There are many ways to measure which variables are important. In this presentation, I'll go over 2 methods that I hope will be helpful for everyone:
...
Please refer to the previous presentation on random forests for a more in-depth explanation of the algorithm. For this presentation, I will simply be talking about how to use the random forest algorithm for help with variable importance. 
...
- Biased towards variables that have a caregory with a high frequency
    - A split that separates the dominant category often leads to a large drop in impurity
...
SHAP values are an application of game theory that has found a role in data science. They measure how much each feature contributes to a model's prediction, on average, across all observations. Predictors with higher SHAP values contribute more to the model's predictions. In general, SHAP values can be used to "look inside" the black box models and provide some explanation as to how the model is making decisions and which features it is putting more weight on.

[@molnar2025]

For our purposes, we will use SHAP values for variable importance. I decided to use a random forest so we can compare the results to the results from the MDI method.

Steps:

1) Compute the SHAP value for each variable at every data point
2) Take the average of the absolute value of the SHAP value for each variable
3) Variables with a higher average SHAP value are more important to the model.
...
- Although it can be used for any model, non-tree based models have slow computational times
